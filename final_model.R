"
---- Notes ----
This script is mostly to plot the fitted and predicted values for a model. To
get metrics and coefficient values use the ensemble script as this averages over
multiple cross-validation runs. The cross-validation in the regularization
introduces randomness so we need to look at the distribution of the metrics and
coefficients.
"
rm(list=ls())
library(dplyr)
...
if(TRUE){ # model
  ...
  if(TRUE){ # Bernoulli model
    ...
    if(TRUE){
      ...
      score.f2 <- round(score.cm[1,1] / (score.cm[1,2] + score.cm[1,1] + score.cm[2,1]), 4)
      ...
      metric.names <- c('BCE', 'Brier', 'AUC-ROC', 'Precision', 'Recall', 'F1', 'F2')
      metrics <- matrix(nrow=1, ncol=7, dimnames=list(NULL,metric.names))
      metrics[1,] <- c(score.bce, score.brier, score.auc.roc, score.precision,
  ...
  if(TRUE){ # predict ZABI
    ...
    if(TRUE){
      ...
      df.bin.sub$p.count <- mu.bin
      df.bin.sub$p.binary <- mu.ber
      df.bin.sub$q50 <- q50
...
outpath <- paste0(res.dir, '/final_model/', WRZ, '_', USE.INDICATOR, '_fits', ENSEMBLE)
write.csv(results, paste0(outpath, '.csv'))
stargazer(results, type='text', out=paste0(outpath, '.txt'))
if(TRUE){ # plot fit
  look.at <- 'FF1'
  ...
  # residuals analysis
  pred <- df.test.subset$q50
  obs <- df.test.subset$y[,2]
  par(mfrow=c(1,1))
  res <- pred - obs
  plot(res, pch=20)
} # plot fit
...
if(FALSE){
  # quarterly statistics (non-overlapping)
  library(slider)

  PERIOD <- "month"
  EVERY <- 2
  ensembles <- unique(df.preds$ensemble)
  f1s <- vector("list", length(ensembles))
  for (i in seq_along(ensembles)) {
    df.ens <- df.preds[df.preds$ensemble == ensembles[i],]
    obs <- vapply(block(df.ens$y[,2], df.ens$date, period=PERIOD, every=EVERY), sum, numeric(1))
    pred <- vapply(block(df.ens$q50, df.ens$date, period=PERIOD, every=EVERY), sum, numeric(1))
    obs <- as.numeric(obs > 0)
    pred <- as.numeric(pred > 0)
    cm <- cm.binary(obs, pred)
    f1 <- cm[1,1] / (cm[1,2] + cm[2,1] + cm[1,1])
    f1s[[i]] <- f1
  }
  f1s <- unlist(f1s);f1s
  mean(f1s)
...
if(FALSE){
  # quarterly statistics (overlapping)
  library(slider)
  ...
  for (i in seq_along(ensembles)) {
    df.ens <- df.preds[df.preds$ensemble == ensembles[i],]
    obs <- slide_period(df.ens$y[,2], df.ens$date, PERIOD, sum, .every=EVERY, .before=1, .after=1)
    pred <- slide_period(df.ens$q50, df.ens$date, PERIOD, sum, .every=EVERY, .before=1, .after=1)
    obs <- as.numeric(obs > 0)
    pred <- as.numeric(pred > 0)
    cm <- cm.binary(obs, pred)
    f1 <- cm[1,1] / (cm[1,2] + cm[2,1] + cm[1,1])
